...
*   **Aladhan Hijri Date Synchronization**: Swaps local calculation fallback rules to integrate the official Aladhan API calendar lookup, adding manual Hijri adjustment date offsets (setting corrections from -3 to +3 days) with local cached backups.
*   **Interactive Namaz (Salah) Guide**: An educational guide detailing exact Arabic text, pronunciation, and translation for Sana, Surah Al-Fatiha, sample Surah choices (Surah Al-Ikhlas, Surah Al-Kawthar), Ruku/Sajdah glorifications, Jalsah sit prayers, Attahiyyat, Durood Ibrahim, and final prayers. Contains a complete prayer Rakat tally table outlining Fard, Sunnah (Mu'akkadah and Ghair Mu'akkadah), Witr, and Nafl counts for all five daily prayers and Jumu'ah.
*   **Hadith & Quran Voice TTS Reader**: Leverages the browser Web Speech Synthesis API to read aloud Arabic text and translations of Hadith cards and last read Quran verses at the click of a speaker button.
*   **Daily Azkar & Duas Companion**: Integrates a curated collection of Morning, Evening, and Daily supplications. Features persistent, localStorage-backed daily counter controls (with visual completion transitions and haptic vibration feedback), a starred favorites bookmarking collection, and built-in Web Speech Synthesis TTS read-aloud spoken translations.
*   **Tasbeeh Analytics & Activity Heatmap**: Displays weekly counters in a progress bar graph and a GitHub-style activity contribution map to track login streaks, log saves, and daily Tasbeeh counts.
